...
if (user) {
  ...
  const recipe = {
    ...
    instructions: "Cook pasta, add tomato sauce, sprinkle cheese, and enjoy!"
  };

  // Save the recipe data under the user's UID
  const userRecipesRef = firebase.database().ref("recipes").child(userUID);
  const newRecipeRef = userRecipesRef.push(); // Generate a unique ID for the new recipe
...
if (user) {
  ...
  userRecipesRef.on("value", snapshot => {
    ...
    // Iterate through recipes and display them in your HTML
  });
} else {
  // No user is signed in
}